    ...
    public class UniversalOR_V9_013_FINAL : Strategy
    {
        ...
        protected override void OnStateChange()
        {
            ...
            else if (State == State.Terminated)
            {
                ...
                StopIpcServer();
                Print("V9 IPC: Server stopped (State.Terminated)");
            }
        }
        #endregion
        ...
        protected override void OnBarUpdate()
        {
            ...
            // V9 DEBUG: Log when processing queued commands
            if (commandQueue.Count > 0)
            {
                Print("V9: OnBarUpdate processing " + commandQueue.Count + " queued command(s)");
            }
            ProcessCommandQueue();
            ManageTightTrailing();
        ...
        private void ListenForRemote()
        {
            try
            {
                ...
                listener.Start();
                Print("V9 IPC: === SERVER STARTED on port " + IpcPort + " ===");

                while (isRunning)
                {
                    if (!listener.Pending()) { Thread.Sleep(10); continue; }

                    Print("V9 IPC: Client connecting...");
                    ...
                    using (NetworkStream stream = client.GetStream())
                    {
                        ...
                        Print("V9 IPC: RAW RECV = [" + message + "]");

                        lock (commandLock) { commandQueue.Enqueue(message); }

                        Print("V9 IPC: Queued. Queue depth = " + commandQueue.Count);
                    }
                }
            }
            catch (Exception ex)
            {
                Print("V9 IPC ERROR: " + ex.GetType().Name + " - " + ex.Message);
            }
        }
