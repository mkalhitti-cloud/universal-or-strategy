...
{
    public class UniversalOR_V9_005_V1 : Strategy
    {
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = "V9_005 V1 - V8.28 Gold Standard Parameters with Max 8pt Stop Hardening";
                Name = "UniversalOR_V9_005_V1";
                ...
                // ========================================
                // V8.28 GOLD STANDARD PARAMETERS
                // ========================================
                ...
                StopMultiplier = 0.5;
                MinimumStop = 1.0;
                MaximumStop = 8.0;  // CRITICAL HARDENING: 8pt max, NOT 12pt from V8.28

                // Target Sizing (4-Target System)
                T1ContractPercent = 20;   // 20% at T1
                T2ContractPercent = 30;   // 30% at T2
                T3ContractPercent = 30;   // 30% at T3
                T4ContractPercent = 20;   // 20% at T4 (runner)

                // Target Math
                Target1FixedPoints = 1.0;  // T1 = Fixed 1pt
                Target2Multiplier = 0.5;   // T2 = 0.5x ATR
                Target3Multiplier = 1.0;   // T3 = 1x ATR
            }
            else if (State == State.DataLoaded)
            {
                StartIpcServer();
                Print("V9_005 V1: Loaded with MaximumStop=" + MaximumStop + " (8pt Hardened)");
            }
        ...
        {
            isRunning = true;
            serverThread = new Thread(ListenForRemote) { IsBackground = true, Name = "V9_005_IPC_Server" };
            serverThread.Start();
            Print("V9_005 V1: Server started on port " + IpcPort);
        }
        ...
        private void ProcessCommandQueue()
        {
            ...
            if (command != null)
            {
                Print("V9_005 V1: Command: " + command);
        ...
        private void ExecuteOrder(string symbol, OrderAction action)
        {
            ...
            if (instrument == null)
            {
                Print("V9_005 V1: ERROR - Instrument not found: " + symbol);
                return;
            }

            // Calculate stop distance (capped at MaximumStop = 8.0)
            ...
            foreach (Account account in Account.All)
            {
                if (account.Name.Contains(AccountPrefix))
                {
                    ...
                    // Calculate T1 target price (fixed 1pt)
                    double targetPrice = action == OrderAction.Buy
                        ? Instrument.MasterInstrument.RoundToTickSize(entryPrice + Target1FixedPoints)
                        : Instrument.MasterInstrument.RoundToTickSize(entryPrice - Target1FixedPoints);

                    // Entry order
                    Order entryOrder = account.CreateOrder(instrument, action, OrderType.Market, TimeInForce.Gtc, 1, 0, 0, "", "V9_005_Entry", null);
                    account.Submit(new[] { entryOrder });

                    // Stop Loss order
                    OrderAction exitAction = action == OrderAction.Buy ? OrderAction.Sell : OrderAction.BuyToCover;
                    Order stopOrder = account.CreateOrder(instrument, exitAction, OrderType.StopMarket, TimeInForce.Gtc, 1, 0, stopPrice, "", "V9_005_Stop", null);
                    account.Submit(new[] { stopOrder });

                    // Profit Target order (T1)
                    Order targetOrder = account.CreateOrder(instrument, exitAction, OrderType.Limit, TimeInForce.Gtc, 1, targetPrice, 0, "", "V9_005_T1", null);
                    account.Submit(new[] { targetOrder });

                    Print("V9_005 V1: " + account.Name + " | Entry=" + entryPrice + " Stop=" + stopPrice + " T1=" + targetPrice + " (MaxStop=" + MaximumStop + ")");
                }
            }
        }
        #endregion
