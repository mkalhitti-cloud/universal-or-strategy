...
namespace SpscBench
{
    ...
    public unsafe sealed class SpscRingV148 : IDisposable {
        ...
        public unsafe bool TryEnqueue(double payload) {
            long prod = *(long*)region;
            long cons = Volatile.Read(ref *(long*)(region + 64));
            if (prod - cons >= capacity) return false;
            byte* slot = region + 128 + (prod & mask) * slotSize;
            Unsafe.CopyBlockUnaligned(
                destination: slot,
                source: (byte*)Unsafe.AsPointer(ref Unsafe.AsRef(in payload)),
                byteCount: (uint)slotSize);
            long shadow = XorShadow.Compute(slot, shadowLength, SHADOW_SALT);
            *(ulong*)(slot + shadowOffset) = shadow;
            Volatile.Write(ref *(long*)region, prod + 1);
            return true;
        }

        public unsafe bool TryDequeue(out double payload) {
            long cons = *(long*)(region + 64);
            long prod = Volatile.Read(ref *(long*)region);
            if (prod == cons) { payload = default; return false; }
            byte* slot = region + 128 + (cons & mask) * slotSize;
            long stamped = *(ulong*)(slot + shadowOffset);
            if (!XorShadow.Validate(slot, shadowLength, stamped, SHADOW_SALT))
            {
                payload = default;
                return false;
            }
            payload = Unsafe.ReadUnaligned (slot);
            Volatile.Write(ref *(long*)(region + 64), cons + 1);
            return true;
        }
