...
{
    public class UniversalOR_V9_008_V1 : Strategy
    {
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = "V9_008 V1 - Floor Trader: Manual Execution (Trim/Flatten) with Multi-Target Architecture";
                Name = "UniversalOR_V9_008_V1";
            ...
            else if (State == State.DataLoaded)
            {
                ...
                Print("V9_008 V1: Loaded | Mode=" + modeDesc + " | TightTrail=" + TightTrailEnabled);
            }
        ...
        {
            isRunning = true;
            serverThread = new Thread(ListenForRemote) { IsBackground = true, Name = "V9_008_IPC_Server" };
            serverThread.Start();
        }
        ...
        private void ProcessCommandQueue()
        {
            ...
            if (command != null)
            {
                ...
                else if (action == "FLATTEN") ExecuteFlatten(symbol);
                else if (action == "TRIM_25") ExecuteTrim(symbol, 0.25);
                else if (action == "TRIM_50") ExecuteTrim(symbol, 0.50);
                else if (action == "BE_PLUS_1") ExecuteBreakevenPlus(symbol, 1.0);
            }
        }
        #endregion
        ...
        private void ExecuteOrder(string symbol, OrderAction action)
        {
            ...
            foreach (Account account in Account.All)
            {
                if (account.Name.Contains(AccountPrefix))
                {
                    ...
                    string signalName = action == OrderAction.Buy ? "V9_008_Long" : "V9_008_Short";
                    ...
                    Order stopOrder = account.CreateOrder(instrument, exitAction, OrderType.StopMarket, TimeInForce.Gtc, totalContracts, 0, stopPrice, "", "V9_008_Stop", null);
                    ...
                    Print(string.Format("V9_008 V1 [{0}]: {1} | {2}x @ {3:F2} | Stop={4:F2} | ATR={5:F2}",
        ...
        private void SubmitTargetOrders(Account account, Instrument instrument, OrderAction exitAction, PositionInfo pos)
        {
            ...
            if (pos.T1Contracts > 0)
            {
                Order t1Order = account.CreateOrder(instrument, exitAction, OrderType.Limit, TimeInForce.Gtc,
                    pos.T1Contracts, pos.Target1Price, 0, "", "V9_008_T1", null);
            ...
            if (pos.T2Contracts > 0)
            {
                Order t2Order = account.CreateOrder(instrument, exitAction, OrderType.Limit, TimeInForce.Gtc,
                    pos.T2Contracts, pos.Target2Price, 0, "", "V9_008_T2", null);
            ...
            if (pos.T3Contracts > 0)
            {
                Order t3Order = account.CreateOrder(instrument, exitAction, OrderType.Limit, TimeInForce.Gtc,
                    pos.T3Contracts, pos.Target3Price, 0, "", "V9_008_T3", null);
            ...
            if (TargetMode == 5 && pos.T4Contracts > 0)
            {
                ...
                Order t4Order = account.CreateOrder(instrument, exitAction, OrderType.Limit, TimeInForce.Gtc,
                    pos.T4Contracts, pos.Target4Price, 0, "", "V9_008_T4", null);
        ...
        #region V9_008: Manual Execution Methods
                ...
                foreach (Position position in account.Positions)
                {
                        ...
                        position.MarketPosition != MarketPosition.Flat)
                    {
                        ...
                        Order flattenOrder = account.CreateOrder(instrument, exitAction, OrderType.Market,
                            TimeInForce.Gtc, qty, 0, 0, "", "V9_008_Flatten", null);
                        account.Submit(new[] { flattenOrder });

                        Print(string.Format("V9_008 V1: FLATTEN | {0} | {1}x {2} @ MKT",
                            account.Name, qty, symbol));
                    }
                }

                // Clear tight trail tracking for this symbol
            ...
            foreach (Account account in Account.All)
            {
                ...
                foreach (Position position in account.Positions)
                {
                        ...
                        position.MarketPosition != MarketPosition.Flat)
                    {
                        ...
                        if (trimQty < 1) trimQty = 1; // Minimum 1 contract
                        ...
                        string trimName = string.Format("V9_008_Trim{0}", (int)(trimPercent * 100));
                        ...
                        Print(string.Format("V9_008 V1: TRIM {0}% | {1} | {2}x {3} @ MKT (was {4}x)",
                            (int)(trimPercent * 100), account.Name, trimQty, symbol, currentQty));

                        // Reduce stop order quantity to match remaining position
            ...
            foreach (Account account in Account.All)
            {
                ...
                foreach (Position position in account.Positions)
                {
                        ...
                        position.MarketPosition != MarketPosition.Flat)
                    {
                        ...
                        // Force update stop to BE+1 (regardless of current stop position)
                        ForceUpdateStop(account, instrument, newStopPrice);

                        Print(string.Format("V9_008 V1: BE+{0} | {1} | {2} | Entry={3:F2} | NewStop={4:F2}",
        ...
        private void UpdateStopQuantity(Account account, Instrument instrument, int newQuantity)
        {
            ...
            {
                if (order.Instrument.FullName == instrument.FullName &&
                    (order.Name.StartsWith("V9_008_Stop") || order.Name == "V9_008_Stop") &&
                    (order.OrderState == OrderState.Accepted || order.OrderState == OrderState.Working))
                {
                    account.Cancel(new[] { order });
                    Order newStopOrder = account.CreateOrder(instrument, order.OrderAction, OrderType.StopMarket,
                        TimeInForce.Gtc, newQuantity, 0, order.StopPrice, "", "V9_008_Stop", null);
        ...
        private void ForceUpdateStop(Account account, Instrument instrument, double newStopPrice)
        {
            foreach (Order order in account.Orders.ToArray())
            {
                    ...
                    (order.OrderState == OrderState.Accepted || order.OrderState == OrderState.Working))
                {
                    account.Cancel(new[] { order });
                    Order newStopOrder = account.CreateOrder(instrument, order.OrderAction, OrderType.StopMarket,
                        TimeInForce.Gtc, order.Quantity, 0, newStopPrice, "", "V9_008_Stop_BE", null);
                    account.Submit(new[] { newStopOrder });
                }
            }
        }
            ...
            foreach (Account account in Account.All)
            {
                ...
                foreach (Position position in account.Positions)
                {
                    ...
                    if (!tightTrailActive[key] && unrealizedPts >= TightTrailTrigger)
                    {
                        ...
                        Print("V9_008 V1: ACTIVATING TIGHT TRAIL for " + account.Name);
                    }
        ...
        private void UpdateStop(Account account, Instrument instrument, double newStopPrice)
        {
            ...
            {
                if (order.Instrument.FullName == instrument.FullName &&
                    (order.Name == "V9_008_Stop" || order.Name == "V9_008_Stop_BE" || order.Name == "V9_008_Stop_Update") &&
                    order.OrderState == OrderState.Accepted)
                {
                    ...
                    if (improvesRisk)
                    {
                        ...
                        account.Cancel(new[] { order });
                        Order newStopOrder = account.CreateOrder(instrument, order.OrderAction, OrderType.StopMarket, TimeInForce.Gtc, order.Quantity, 0, roundedStop, "", "V9_008_Stop_Update", null);
                        account.Submit(new[] { newStopOrder });
                    }
                }
            }
