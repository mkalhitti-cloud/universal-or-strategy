...
app.state.config = {
    ...
    "agent_runtime_object_path": "..agent_runtime_app.agent_runtime",
}
...
class WebSocketToQueueAdapter:
    ...
    def __init__(
        ...
        Args:
            ...
            agent_runtime: The agent engine instance with bidi_stream_query method (None if using remote)
            remote_config: Remote agent engine configuration (project_id, location, remote_agent_runtime_id)
        """
    ...
    def _transform_remote_agent_runtime_response(self, response: dict) -> dict:
        """Transform remote Agent Runtime bidiStreamOutput to ADK Event format for frontend."""
        # Check if this is a remote Agent Runtime bidiStreamOutput
    ...
    async def run_agent_runtime(self) -> None:
        ...
        try:
            if self.agent_runtime is not None:
                ...
                async for response in self.agent_runtime.bidi_stream_query(
                    self.input_queue
                ):
                ...
                await self.run_remote_agent_runtime(
                    project_id=self.remote_config["project_id"],
                    location=self.remote_config["location"],
                    remote_agent_runtime_id=self.remote_config[
    ...
    async def run_remote_agent_runtime(
        self, project_id: str, location: str, remote_agent_runtime_id: str
    ) -> None:
        ...
        async with client.aio.live.agent_engines.connect(
            agent_runtime=remote_agent_runtime_id,
            ...
            async def receive_from_remote() -> None:
                while True:
                    try:
                        ...
                        if response is not None:
                            # Transform remote Agent Runtime bidiStreamOutput format to frontend format
                            transformed = self._transform_remote_agent_runtime_response(
                                response
                            )
                            if transformed:
...
def get_connect_and_run_callable(
    ...
    async def connect_and_run() -> None:
        if config["use_remote_agent"]:
            # Remote agent engine mode
            logging.info(
                f"Connecting to remote agent engine: {config['remote_agent_runtime_id']}"
            )
            remote_config = {
                "project_id": config["project_id"],
                "location": config["location"],
                "remote_agent_runtime_id": config["remote_agent_runtime_id"],
            }
            adapter = WebSocketToQueueAdapter(
                websocket, agent_runtime=None, remote_config=remote_config
            )
        else:
            # Local agent engine mode
            # Dynamically import the pre-configured agent_runtime object
            agent_runtime = _dynamic_import(config["agent_runtime_object_path"])
            logging.info(
                f"Starting local agent engine with object: {type(agent_runtime).__name__}"
            )

            adapter = WebSocketToQueueAdapter(websocket, agent_runtime)

        logging.info("Starting bidirectional communication with agent engine")
        await asyncio.gather(
            adapter.receive_from_client(),
            adapter.run_agent_runtime(),
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Agent Runtime Proxy Server")
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--agent-engine-object",
        type=str,
        default="..agent_runtime_app.agent_runtime",
    ...
    config: dict[str, Any] = {
        ...
        "agent_runtime_object_path": args.agent_runtime_object,
    }
    ...
    if args.mode == "remote":
        ...
        if not args.remote_id:
            ...
            if deployment_metadata_path.exists():
                with open(deployment_metadata_path) as f:
                    ...
                    config["remote_agent_runtime_id"] = metadata.get(
                        "remote_agent_runtime_id"
                    )
                    if not config["remote_agent_runtime_id"]:
                        parser.error(
                            "No remote_agent_runtime_id found in deployment_metadata.json"
                        )
            ...
            config["remote_agent_runtime_id"] = args.remote_id
        ...
        if not args.project_id:
            ...
            remote_id: str = config["remote_agent_runtime_id"]
            match = re.match(
        ...
        print(f"  Remote Agent Runtime ID: {config['remote_agent_runtime_id']}")
        print(f"  Project ID: {config['project_id']}")
        print(f"  Location: {config['location']}")
    else:
        print("Starting server in LOCAL mode")
        print(f"  Using agent: {config['local_agent_path']}")
        print(f"  Using agent engine object: {config['agent_runtime_object_path']}")
